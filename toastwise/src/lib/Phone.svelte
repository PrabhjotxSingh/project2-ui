<script>
    import { currentTimer, selectedBread, toasterStatus, favorites } from './store';
  ...
  function addFavorite() {
    if (newTitle && newBreadType && newToastTime > 0) {
      favorites.update(favs => [...favs, { title: newTitle, breadType: newBreadType, time: newToastTime }]);
          ...
          {#each $favorites as favorite, index}
              <li class="favorite-item">
                  <div class="item-content">
                      ...
                      <span class="favorite-details">{favorite.breadType} - {favorite.time} min</span>
                  </div>
                  <button class="delete-button" on:click={() => deleteFavorite(index)}>-</button>
              </li>
          {/each}
      </ul>

      <button class="back-button" on:click={toggleEditScreen}>Done</button> <!-- Done button to go back -->
...
<div class="popup">
    ...
    <div class="popup-body">
        <input class="input-field" type="text" bind:value={newTitle} placeholder="Title" />
        <input class="input-field" type="text" bind:value={newBreadType} placeholder="Type of Bread" />
        <input class="input-field" type="number" bind:value={newToastTime} min="1" placeholder="Timer" />
...
{/if}
  ...
  <div>
    ...
    <button class="edit-button" on:click={toggleEditScreen}>Edit Favorites</button>
</div>
  {/if}
</div>
